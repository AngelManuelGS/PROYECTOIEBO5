        ...
        <div class="card shadow-sm border-2" style="border-color: var(--color-primary); border-radius: 8px;">
            ...
            <div class="card-body">
                ...
                <p><strong>Fecha de Registro del Cliente:</strong>
                    {{ $venta->cliente?->created_at ? $venta->cliente->created_at->format('d/m/Y H:i:s') : 'N/A' }}
                </p>
                <p><strong>Fecha de Venta:</strong>
                    {{ $venta->created_at ? $venta->created_at->format('d/m/Y H:i:s') : 'N/A' }}
                </p>
                <p><strong>Monto Total:</strong>
                    <span class="text-success" style="font-size: 1.3rem; font-weight: bold;">
                ...
                <p><strong>Estado:</strong>
                    <span
                        class="badge
                    {{ $venta->estado === 'pendiente' ? 'bg-warning' : ($venta->estado === 'aprobado' ? 'bg-success' : 'bg-danger') }} ">
                    ...
                    <table class="table table-bordered table-hover text-center">
                        ...
                        <tbody>
                            @foreach ($venta->detalleventa as $detalle)
                                <tr>
                                    <td>{{ $detalle->producto->nombre ?? 'Productos' }}</td>
